...
void loop(){
 ...
 webSocket.loop();
  ...
  if(buttonPin0.uniquePress()){    
     char jsonC[] = "{\"name\":\"VEDAT BALOĞLU\", \"year\":\"2020\",\"city\":\"İSTANBUL\"}";
     webSocket.broadcastTXT(jsonC, sizeof(jsonC)-1);
  } 

  if(buttonPin1.uniquePress()){    
     String strTest = "{\"name\":\"Altium Designer Projects\", \"year\":\"2019\",\"city\":\"Turkey\"}";
     strTest.toCharArray(payloadConvert, strTest.length()+1);
     webSocket.broadcastTXT(payloadConvert, strTest.length());
  }
  
  if(buttonPin2.uniquePress()){  
     String strTest = "{\"adc\":\"1234\"}";
     strTest.toCharArray(payloadConvert, strTest.length()+1);
     webSocket.broadcastTXT(payloadConvert, strTest.length());
  } 
